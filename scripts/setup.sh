...
echo "   Frontend:    http://study-ckad.local/"
echo "   Backend API: http://study-ckad.local/api/v1/tasks"
echo "   Dex (OIDC):  http://dex.study-ckad.local/"
echo ""
echo -e "🔐 ${YELLOW}認証情報（Dex ローカルユーザー）:${NC}"
echo "   admin@example.com / password"
echo "   user@example.com  / password"
echo ""
echo -e "⚠️  ${YELLOW}/etc/hosts に以下を追加してください:${NC}"
echo "   ${MINIKUBE_IP} study-ckad.local dex.study-ckad.local"
echo ""
